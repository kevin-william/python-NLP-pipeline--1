...
class ModeloNMF:
    ...
    def treinar(self, matriz_tfidf):
        logger.info("Treinando NMF com %d topicos sobre matriz TF-IDF shape=%s", self.num_topicos, matriz_tfidf.shape)
        self.modelo = NMF(n_components=self.num_topicos, **self.params)
        self.matriz_documento_topico = self.modelo.fit_transform(matriz_tfidf)
        logger.info("NMF treinado. Matriz doc-topico shape=%s", self.matriz_documento_topico.shape)
        return self

    def obter_topicos(self, vocabulario, top_n=10):
        if self.modelo is None:
            raise RuntimeError("Modelo NMF nao foi treinado ainda")
