...
class ModeloLDA:
    ...
    def treinar(self, matriz_bow):
        logger.info("Treinando LDA com %d topicos sobre matriz BoW shape=%s", self.num_topicos, matriz_bow.shape)
        self.modelo = LatentDirichletAllocation(n_components=self.num_topicos, **self.params)
        self.matriz_documento_topico = self.modelo.fit_transform(matriz_bow)
        logger.info("LDA treinado. Matriz doc-topico shape=%s", self.matriz_documento_topico.shape)
        return self
    ...
    def obter_perplexidade(self, matriz_bow):
        if self.modelo is None:
            raise RuntimeError("Modelo LDA nao foi treinado ainda")
        perplexidade = self.modelo.perplexity(matriz_bow)
